...
async function processIntent(
  ...
  switch (intent) {
    ...
    case 'proporcionar_datos': {
      // Si ya teníamos un flujo de proveedores activo o se detectó la intención
      if (session.pendingAction === 'proveedores' || intent === 'proveedores') {
        session.pendingAction = 'proveedores'
        
        // Detectar si menciona un archivo
        if (message.toLowerCase().includes('pdf') || message.toLowerCase().includes('archivo') || message.toLowerCase().includes('presentación')) {
          console.log('[Chat API] 📄 El usuario menciona un archivo en el flujo de proveedores');
        }

        // Ya NO asignamos el mensaje automáticamente por orden. 
        // Dejamos que la IA extraiga lo que pueda del mensaje arriba (en detectIntent -> session.collectedData)
        return await handleActionRequest('proveedores', session, genAI, history, messageId)
      }
      // Primera vez que detectamos proveedor
      session.pendingAction = 'proveedores'
      return await handleActionRequest('proveedores', session, genAI, history, messageId)
    }
